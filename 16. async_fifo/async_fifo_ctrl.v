`timescale 1ns / 1ps
// 模块: async_fifo_ctrl
// 说明: 异步 FIFO 控制器，负责写/读指针管理、灰码同步、空满标志计算以及写/读深度统计。
// 主要参数:
//  - DATA_WIDTH: 数据宽度
//  - ADDR_WIDTH: 地址宽度
//  - FULL_AHEAD: almost_full 阈值配置
// 主要端口:
//  - reset: 复位（高电平有效）
//  - wrclk/wren/wrdata: 写端接口
//  - rdclk/rden: 读端接口
//  - ram_dout/ram_wraddr/ram_rdaddr/ram_din/ram_wren: 与双口 RAM 的接口
// 说明/备注:
//  - 使用二进制-灰码互转进行跨时钟域指针同步，保证空满判定的可靠性。
// -----------------------------------------------------------------------------
